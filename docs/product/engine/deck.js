...
/* 표기 형식 — 원천의 숫자를 **덱이 쓰던 모양 그대로** 찍는다.
   ★ 값은 원천에서만 오고, 모양만 여기서 정한다. 둘을 섞으면 원천 단일성이 무너진다. */
/** 소수 자릿수를 맞추고 꼬리 0 을 턴다. dp 가 없으면 값을 그대로 쓴다. */
function trimTo(n, dp) {
  if (dp === null || dp === undefined || dp === '') return String(n);
  return n.toFixed(+dp).replace(/(\.\d*?)0+$/, '$1').replace(/\.$/, '');
}
/** 천단위 쉼표 (덱이 10,000 N · 2,070.12 처럼 쓴다) */
function group(s) {
...
var BIND_FMT = {
  /* 값 — 유니코드 마이너스 · 천단위 쉼표 · (자릿수 지정이 있으면) 반올림 */
  num: function (v, dp) {
    var n = Number(v);
    if (isNaN(n)) return String(v).replace(/-/g, '−');
    return group(trimTo(Math.abs(n), dp)).replace(/^/, n < 0 ? '−' : '');
  },
  /* 부호를 항상 보이는 백분율 — +0.0002% · −1.994% · 0% */
  spct: function (v, dp) {
    var n = Number(v); if (isNaN(n)) return '—';
    return (n > 0 ? '+' : n < 0 ? '−' : '') + trimTo(Math.abs(n), dp) + '%';
  },
  /* 허용오차 — 1% · 0.05% · 0.000001% */
  pct0: function (v, dp) { var n = Number(v); return isNaN(n) ? '—' : trimTo(Math.abs(n), dp) + '%'; },
};
...
function fillBinds(root) {
  root.querySelectorAll('[data-bind]').forEach(function (n) {
    ...
    /* `_` 는 summary, 그 밖은 records 아래. 그 뒤로는 점으로 얼마든지 파고든다
       (예: SB1.computed.0.strix) — 대조값이 computed 안에 있기 때문이다. */
    var v = (p[0] === '_') ? D.summary : (D.records || {})[p[0]];
    for (var i = 1; i < p.length && v != null; i++) v = v[p[i]];
    if (v === undefined || v === null) { n.textContent = '—'; return; }
    /* data-dp = 화면이 쓰던 소수 자릿수. 값은 원천에서, 표기 정밀도만 덱이 정한다. */
    var f = BIND_FMT[n.getAttribute('data-fmt')];
    n.textContent = f ? f(v, n.getAttribute('data-dp')) : (/Pct$/.test(p[p.length - 1]) ? pct(v) : v);
  });
}
...
function renderSlide(def, index, total) {
  ...
  var c    = el('div', 'canvas');
  /* ★ 슬라이드 단위 클래스는 캔버스가 진다 (제품 덱 Edition B 스킨이 여기에 걸린다).
     `.canvas.screen .point p` 처럼 레이아웃마다 다른 규칙이 이 한 줄로 살아난다.
     스킨을 쓰지 않는 덱은 def.cls 가 없으므로 아무 영향도 없다. */
  if (def.cls) c.className += ' ' + def.cls;
  c.dataset.id = def.id;
  ...
  if (def.foot !== false) {
    ...
    /* def.foot 이 [x,y,w,h] 면 그 자리에 둔다 — 이식한 덱은 원본에서 잰 값을 갖고 온다.
       (없으면 기존 기본값 그대로다. 다른 덱은 영향받지 않는다) */
    var fb = Array.isArray(def.foot) ? def.foot : null;
    var pad = Math.round(cv.w * 0.07);
    f.style.left   = (fb ? fb[0] : pad) + 'px';
    f.style.top    = (fb ? fb[1] : Math.round(cv.h * 0.902)) + 'px';
    f.style.width  = (fb ? fb[2] : cv.w - pad * 2) + 'px';
    f.style.height = (fb ? fb[3] : Math.round(cv.h * 0.045)) + 'px';
    f.dataset.role = 'foot';
    var note = def.note || (Deck.data.meta || {}).footerNote || '';
    if (fb) {
      /* 이식 덱은 원본 각주 줄의 생김새를 그대로 쓴다 — 스킨이 그 마크업에 걸려 있다.
         ⚠ 왼쪽 글에 class 를 붙이지 말 것: deck.css 의 .fnote 색이 스킨을 이긴다. */
      f.innerHTML = '<footer><span>' + esc(note) + '</span>'
                  + '<div class="page">' + (Deck.pageLabel(def) || '') + '</div></footer>';
    } else {
...
/* ── 쪽번호 ─────────────────────────────────────────────────────────────
 *  🚨 "몇 번째 장인가" 가 아니다. 데이터가 규칙을 정한다.
 *      def.foot === false   각주 줄 자체가 없다 (목차 · 간지)
 *      def.pgnum === false  줄은 있고 번호만 없다 (표지 · 마지막 장)
 *      def.apx              부록은 따로 센다 — A01 / <meta.appendixLabel>
 *      그 밖                번호가 붙는 장끼리만 센다  NN / 총수
 *  이렇게 해야 장을 끼워 넣거나 옮겨도 번호가 저절로 맞는다. 손으로 적으면
 *  순서를 바꾼 순간 조용히 틀린 번호가 인쇄된다.
 * -------------------------------------------------------------------- */
Deck.pageLabels = null;
function buildPageLabels(defs) {
  var meta = Deck.data.meta || {};
  var pad = function (n) { return n < 10 ? '0' + n : String(n); };
  var counts = function (d) { return d.foot !== false && d.pgnum !== false; };
  var body = defs.filter(function (d) { return counts(d) && !d.apx; });
  var apx  = defs.filter(function (d) { return counts(d) && d.apx; });
  var m = {};
  body.forEach(function (d, i) { m[d.id] = pad(i + 1) + '<span>/ ' + pad(body.length) + '</span>'; });
  apx.forEach(function (d, i) {
    m[d.id] = 'A' + pad(i + 1) + '<span>/ ' + esc(meta.appendixLabel || 'APPENDIX') + '</span>';
  });
  Deck.pageLabels = m;
}
Deck.pageLabel = function (def) { return (Deck.pageLabels || {})[def.id] || ''; };
...
Deck.render = function () {
  ...
  buildPageLabels(defs);                 /* 쪽번호는 렌더 직전에 다시 센다 */
